...
def scale_numeric_columns(dfs: list, target_variable: str) -> list:
    ...
    for df in dfs:
        ...
        cols_to_scale = [col for col in numeric_cols if col not in ['IDfg', target_variable]] # Scaling all numeric columns except the player ID & target variable   
...
def create_baseline(x_train: pd.DataFrame, x_test: pd.DataFrame, y_train: pd.DataFrame, y_test: pd.DataFrame):
    """
...
def tune_model(x_train: pd.DataFrame, x_test: pd.DataFrame, y_train: pd.DataFrame, y_test: pd.DataFrame, 
               space: dict, n_estimators_list: list, max_depth_list: list):
...
def create_model(x_train: pd.DataFrame, x_test: pd.DataFrame, y_train: pd.DataFrame, y_test: pd.DataFrame, final_params: dict):
    """
