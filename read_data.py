...
from helper import calc_fantasy_points_batting, calc_fantasy_points_pitching, add_suffix_to_columns, save_data, load_training_data, split_name
import requests
...
def pull_projections(url: str):
    ...
    df["Team"] = df["Player"].str.extract(r"\(([^-]+) -")[0].str.strip() # Extract team, strings following the "(" and before the "-"
    df["Positions"] = df["Player"].str.extract(r"- (.+)\)")[0].str.strip() # Extract positions, strings following the "-" and before the ")"
    df["Player"] = df["Player"].str.extract(r"^(.+?) \(")[0] # Extract player name, strings at the beginning of the string and before the "("
...
def pull_data(start_year: int, end_year: int, agg_years: int, batting_stat_cols: list, pitching_stat_cols: list, save_results: bool) -> tuple:
    """
    ...
    for year in years:
        ...
        batting_df_future = (
            batting_stats(start_season = end_year_future, # Selecting a single season for most recent stats
                            qual = 50,
                            split_seasons = True)
        ...
        # Pulling batting stats
        batting_df_current = (
            batting_stats(start_season = year, # Selecting a single season for most recent stats
                            qual = 50,
                            split_seasons = True)
                            .filter(items = batting_stat_cols)
        )
        ...
        pitching_df_future = (
            pitching_stats(start_season = end_year_future, # Selecting a single season for most recent stats
                            qual = 20,
                            split_seasons = True)
        ...
        # Pulling pitching stats
        pitching_df_current = (
            pitching_stats(start_season = year, # Selecting a single season for most recent stats
                            qual = 20,
                            split_seasons = True)
                            .filter(items = pitching_stat_cols)
        )
