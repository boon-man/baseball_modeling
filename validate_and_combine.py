...
def compile_predictions(complete_df: pd.DataFrame, test_df: pd.DataFrame,  y_test: pd.DataFrame, y_pred: pd):
    """
    ...
    comp_df2['diff'] = abs(comp_df2['fantasy_points_future'] - comp_df2['fantasy_points_pred'])

    results = comp_df2[['Name', 'fantasy_points_future', 'fantasy_points_pred', 'diff', 'Season', 
                        'Age', 'fantasy_points', 'fantasy_points_prior']].sort_values(by='diff', ascending=False)

    return results

def combine_projections(prediction_df: pd.DataFrame, projection_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    combined = prediction_df.merge(projection_df, on=['first_name', 'last_name'], how='outer')

    # Filter to rows where IDfg contains duplicate values (excluding NaN)
    duplicates = combined[combined['IDfg'].notna() & combined.duplicated('IDfg', keep=False)]
